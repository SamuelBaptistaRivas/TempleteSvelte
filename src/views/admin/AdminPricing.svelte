  ...
  function reload() {
    ...
    services.subscriptionProducts
      ...
      .then((response: SubscriptionProductDto[]) => {
        ...
        items = products;
        const notInDatabase = products.filter(
          (f) => f.id === undefined || f.id === ""
        );
...
<svelte:head>
  <title>{$t("admin.pricing.title")} | {$t("shared.websiteTitle")}</title>
</svelte:head>
    ...
    >
      ...
      <div class="flex items-center space-x-2 h-9">
        <ButtonSecondary to="/pricing" target="_blank">
          ...
          {:else}
            <div>{$t("shared.view")}</div>{/if}
        </ButtonSecondary>
      </div>
    </div>
  </div>
  <div
    class="pt-2 space-y-2 mx-auto px-4 sm:px-6 lg:px-8 max-w-5xl xl:max-w-7xl overflow-auto"
  >
    {#if loading}
      <Loading />
    {:else}
                  ...
                  <tbody class="bg-white divide-y divide-gray-200">
                    {#each items as item}
                      <tr class="text-gray-600">
                        ...
                        <td class="truncate px-3 py-3 text-sm leading-3"
                          >{#if item.badge}
                            <div>{$t(item.badge)}</div>{/if}</td
                        >
                        <td class="truncate px-3 py-3 text-sm leading-3"
                          >{item.active
                            ? $t("shared.active")
                            : $t("shared.inactive")}</td
                        >
                        <td class="truncate px-3 py-3 text-sm leading-3">
                          {#if !item.maxWorkspaces || item.maxWorkspaces === 0}
                            {$t("shared.unlimited")}
                          {:else}
                            {item.maxWorkspaces}{/if}
                        </td>
                        <td class="truncate px-3 py-3 text-sm leading-3">
                          {#if !item.maxUsers || item.maxUsers === 0}
                            {$t("shared.unlimited")}
                          {:else}
                            {item.maxUsers}{/if}
                        </td>
                        <td class="truncate px-3 py-3 text-sm leading-3">
                          {#if !item.maxLinks || item.maxLinks === 0}
                            {$t("shared.unlimited")}
                          {:else}
                            {item.maxLinks}{/if}
                        </td>
                        <td class="truncate px-3 py-3 text-sm leading-3">
                          {#if !item.maxStorage || item.maxStorage === 0}
                            {$t("shared.unlimited")}
                          {:else}
                            {item.maxStorage / 1024}{/if}
                          {#if item.maxStorage > 0}
                            <span>{$t("shared.storage.gb")}</span>{/if}
                        </td>
                        <td class="truncate px-3 py-3 text-sm leading-3">
                          {!item.monthlyContracts || item.maxLinks === 0
                            ? $t("shared.unlimited")
                            : item.monthlyContracts}
                        </td>
                        <td
