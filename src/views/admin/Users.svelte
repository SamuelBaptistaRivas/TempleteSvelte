<script lang="ts">
  ...
  function changePassword(user: UserDto) {
    const password = prompt(
      $t("settings.profile.changePassword") + " - " + user.email
    );
...
<div>
                              ...
                              >
                                <div class="flex items-center space-x-2">
                                  <ButtonTertiary
                                    ...
                                    on:click={() => impersonate(item)}
                                  >
                                    {$t("models.user.impersonate")}
                                  </ButtonTertiary>
                                  <ButtonTertiary
                                    disabled={item.type === 0}
                                    on:click={() => changePassword(item)}
                                  >
  ...
  <ConfirmModal
    ...
    bind:value={userToBeDeleted}
    on:yes={confirmDeleteUser}
  />
</div>
