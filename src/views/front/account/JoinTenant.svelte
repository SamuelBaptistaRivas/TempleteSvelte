      ...
      {#if requested}
        <div>
          <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
            ...
            >
              <h2 class="mt-6 text-center text-3xl leading-9 font-bold">
          ...
          {#if tenant && tenant.name}
            <div>
              ...
              <div class="mt-6 text-center text-lg leading-9 font-bold">
                {#if accepted}
                  <div>
                    {$t("account.invitation.acceptedUser", {
                      values: { p1: tenant.name },
                    })}{" "}
                    <p class="font-normal text-base">
                      {$t("account.forgot.enterPassword")}
                    </p>
                  </div>
                {:else}
                  <div>
                    ...
                    <p
                      class="mt-2 text-center text-sm leading-5 text-gray-500 max-w"
                    >
                      ...
                      >
                        <Link to="/login"
                          >{$t("account.register.clickHereToLogin")}</Link
                        >
                      </span>
                    </p>
                  </div>
                  ...
                  <form on:submit|preventDefault={accept} class="sm:w-96">
                    ...
                    <div class="mt-6">
                      <span class="block w-full rounded-sm shadow-sm">
                        ...
                        >
                          {#if accepted}
                            <span>{$t("shared.enter")}</span>
                          {:else}
                            <span>{$t("shared.request")}</span>{/if}
                        </LoadingButton>
                      </span>
                    </div>
                  </form>
