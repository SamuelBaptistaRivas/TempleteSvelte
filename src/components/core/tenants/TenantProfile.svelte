<script lang="ts">
  ...
  function confirmedDelete() {
    ...
    services.tenants
      .adminDelete(id)
      .then(() => {
        successModalDeleted?.show(
          $t("shared.deleted"),
          $t("app.tenants.actions.deleted")
        );
      })
      .catch((error) => {
              ...
              <div class="flex items-center space-x-5 truncate">
                ...
                <div class="truncate">
                  ...
                  <p class="text-sm font-medium text-gray-500 truncate">
                    ...
                    {$t("shared.in")}
                    {#if item.createdAt}
                      <time>{dateMonthDayYear(item.createdAt)}</time>{/if}
                  </p>
                </div>
              </div>
            </div>
                    ...
                    <div class="border-t border-gray-200 px-4 py-5 sm:px-6">
                      ...
                      >
                        ...
                        <div class="sm:col-span-1">
                          ...
                          <dd class="mt-1 text-sm text-gray-900">
                            {#if item.workspaces}
                              <span>{item.workspaces.length}</span>{/if}
                          </dd>
                        </div>
                        <div class="sm:col-span-2">
                          ...
                          <dd class="mt-1 text-sm text-gray-900">
                            {#if item.users}
                              <span>{item.users.length}</span>{/if}
                          </dd>
                        </div>
                      </dl>
                    </div>
