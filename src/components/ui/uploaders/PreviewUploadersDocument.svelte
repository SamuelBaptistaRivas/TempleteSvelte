<script lang="ts">
  ...
  function droppedDocuments(event: any) {
    ...
    alert(
      `@droppedDocuments ${filesBase64.length} files: ` +
        filesBase64.map((f) => f.base64.substr(0, 30) + "...")
    );
  }
  function droppedDocument(event: any) {
    ...
    alert(
      `@droppedDocument: base64 [${base64.substr(0, 30)}...], file [name (${
        file.name
      }), size (${file.size})]`
    );
  }
...
<div class="space-y-2 w-full">
  <UploadDocument
    multiple={true}
    on:droppedFiles={droppedDocuments}
    on:dropped={droppedDocument}
  />
</div>
