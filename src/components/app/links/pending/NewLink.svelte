<script lang="ts">
    ...
    services.links
      ...
      .catch(() => {
        confirmInviteNewUserModal?.show(
          ...
          $t("shared.invite"),
          $t("shared.cancel"),
          $t("app.links.invitation.userNotRegistered", {
            values: { p1: email },
          })
        );
  ...
  function searchWorkspace() {
    services.links
      ...
      .catch(() => {
        errorModal?.show(
          $t("shared.error"),
          $t("app.links.invitation.notFound", {
            values: { p1: email, p2: workspaceName },
          })
        );
      })
      .finally(() => {
  ...
  function createLink() {
    const confirmText = workspace?.id
      ? $t("app.links.link")
      : $t("shared.invite");
    const inviteText = workspace?.id ? "" : $t("app.links.invitation.invite");
    if (asProvider) {
      confirmCreateLinkModal?.show(
        $t("app.clients.new.add"),
        confirmText,
        $t("shared.cancel"),
        inviteText
      );
    ...
    services.links
      ...
      .then(() => {
        successModal?.show(
          ...
          $t("app.links.pending.invitationSentDescription", {
            values: { p1: email },
          })
        );
      })
...
<div>
            ...
            >
              ...
              <div class="mt-3 space-y-4">
                <h3 class="font-bold text-lg pb-2 border-b border-gray-100">
                  {#if asProvider}
                    <span>{$t("app.links.newClient")}</span>
                  {:else}
                    <span>{$t("app.links.newProvider")}</span>{/if}
                </h3>
                <div>
                  <div class="mx-auto">
                      ...
                      <div>
                        ...
                        {:else if maxLinksReached}
                          <div>
                            <WarningBanner
                              ...
                              text={$t(
                                "app.subscription.errors.limitReachedLinks",
                                { values: { p1: maxLinks } }
                              )}
                            />
                          </div>
                        {:else}
                          <div class="bg-white rounded space-y-3">
                            <form
                              on:submit|preventDefault={linkUserWorkspace}
                              class="space-y-4"
                            >
                              {#if selectType}
                                <div
                              ...
                              <div class="sm:col-span-6 grid grid-cols-2 gap-2">
                                <div>
                                  ...
                                  <div
                                    class="mt-1 flex rounded-md shadow-sm w-full"
                                  >
                                    <input
                                      type="email"
                                      name="email"
                              ...
                              >
                                ...
                                <button
                                  ...
                                  class={classNames(
                                    ...
                                    (loadingSearching || loadingLinking) &&
                            ...
                            {#if workspace}
                              <div>
                                ...
                                >
                                  ...
                                  >
                                    <div
                                      class="flex items-center space-x-2 text-theme-800 truncate"
                                    >
                                      <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        class="h-9 w-9"
  ...
  <ConfirmModal
    bind:this={confirmInviteNewUserModal}
    on:yes={confirmInviteNewUser}
  />
  <SuccessModal
    bind:this={successModal}
    on:closed={createdLink}
