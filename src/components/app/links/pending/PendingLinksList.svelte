<script lang="ts">
  ...
  function accept(item: LinkDto) {
    ...
    if (modalAccept) {
      selectedLink = item;
      modalAccept.show(
        $t("app.links.invitation.confirmAccept", {
          values: { p1: whoAmIName },
        }),
        $t("shared.accept"),
        $t("shared.back"),
        $t("app.links.invitation.acceptWarning", {
          values: { p1: inviterWorkspace(item).name },
        })
      );
    }
  }
...
<div class="max-w-lg mx-auto pb-12">
                ...
                <div class="w-full space-y-2">
                  {#if item.createdByWorkspaceId !== currentWorkspaceId && item.createdByUser}
                    <div class="flex items-center justify-between space-x-3">
                      ...
                      >
                        {item.createdByUser.firstName} ({item.createdByUser
                          .email}) {$t("app.links.invitation.hasSentYou")}.
                      </p>
                    </div>
                  {/if}
                  <div class="flex items-center justify-between space-x-3">
                    <h3 class="text-gray-900 text-sm font-medium truncate">
                      {#if whoAmI(item) === 0}
                        <span>{item.clientWorkspace.name}</span>
                      {:else}
                        <span>{item.providerWorkspace.name}</span>{/if}
                    </h3>
                    ...
                    <div class="w-full">
                      {#if item.status === 0}
                        ...
                        >
                          ...
                          <span class="ml-3"
                            >{$t("app.links.pending.invitationSent")}</span
                          >
                        </div>
                      {/if}
                      {#if item.status === 2}
  ...
  {/if}
  <ConfirmModal
    bind:this={modalAccept}
    bind:value={selectedLink}
    on:yes={accepted}
  />
  <ConfirmModal
    bind:this={modalReject}
    bind:value={selectedLink}
    on:yes={rejected}
  />
